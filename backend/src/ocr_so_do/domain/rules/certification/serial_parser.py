...
class SerialParser:
    ...
    def cross_validate_serial(
        ...
        if clean_ocr and file_serial:
            ...
            # Trong luồng hồ sơ hiện hành tên file là mã GCN do người dùng đặt.
            # Nếu OCR đọc khác dù vẫn đúng định dạng, ưu tiên mã tên file để
            # tránh một chữ số sai (BK 196001 thay vì BK 198001) đi vào Excel.
            return file_serial, 0.90, f"OCR ({clean_ocr}) khác tên file chuẩn ({file_serial}) -> chọn tên file"
