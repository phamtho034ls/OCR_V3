    ...
    def find_anchors(
        ...
        for box in ocr_boxes:
            ...
            for label in search_labels:
                ...
                # Guard: Title blocks (GIẤY CHỨNG NHẬN, CỘNG HÒA, QUYỀN SỞ HỮU...) cannot match so_vao_so
                if any(k in label.lower() for k in ["vào sổ", "vao so", "sổ cấp", "so cap"]):
                    if not any(k in text.lower() for k in ["sổ", "so", "vào", "vao"]):
                        continue
                    if any(k in text.upper() for k in ["GIẤY CHỨNG NHẬN", "CỘNG HÒA", "ĐỘC LẬP", "QUYỀN SỞ HỮU"]) and not any(k in text.lower() for k in ["vào sổ", "vao so"]):
                        continue
