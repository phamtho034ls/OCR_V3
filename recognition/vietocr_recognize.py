...
class VietOCRRecognizer:
        ...
        try:
            ...
            if valid_imgs:
                batch_preds = self._predictor.predict_batch(
                    valid_imgs, return_prob=True
                )
                # VietOCR predict_batch(return_prob=True) trả về (list_texts, list_probs)
                if isinstance(batch_preds, tuple) and len(batch_preds) == 2:
                    valid_pairs = list(zip(batch_preds[0], [float(p) for p in batch_preds[1]]))
                elif isinstance(batch_preds, list):
                    valid_pairs = [(str(p), 0.90) for p in batch_preds]
                else:
                    valid_pairs = [(str(batch_preds), 0.90)]
            else:
                valid_pairs = []
    ...
    def _init_predictor(Predictor, Cfg, model_name: str, device: str):
        ...
        try:
            ...
            local_cfg_path = Path(__file__).parent.parent / "configs" / f"{model_name}_config.json"
            ...
            # Ưu tiên load weights trực tiếp từ thư mục dự án trên ổ D thay vì tải vào ổ C
            weights_path = Path(__file__).parent.parent / "weights" / f"{model_name}.pth"
